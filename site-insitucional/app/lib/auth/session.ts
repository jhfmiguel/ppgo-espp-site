...
function obterSegredoSessao(): string {
  ...
  if (configurado) {
    if (process.env.NODE_ENV === "production" && configurado.length < 32) {
      throw new Error(
        "ESPP_SESSION_SECRET deve possuir pelo menos 32 caracteres em producao.",
      );
    }
  ...
  if (process.env.NODE_ENV === "production") {
    throw new Error(
      "ESPP_SESSION_SECRET e obrigatorio em producao. Configure um segredo forte no ambiente.",
    );
  }
